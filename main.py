...
def get_cars(request: Request, number:Optional[str] = Query("10",max_length=3)):
    ...
    return templates.TemplateResponse("index.html",{"request":request, "cars":response, "title": "All Cars!"})

@app.post("/search", response_class=RedirectResponse)
def search_car(id:str = Form(...)):
...
def get_car_by_id(request: Request, id:int = Path(...,ge=0,lt=1000)):
    car = cars.get(id)
    response = templates.TemplateResponse("search.html", {"request": request, "car":car, "id":id, "title":"Searched Car"})
    if not car:
...
def add_cars(make: Optional[str] = Form(...),
